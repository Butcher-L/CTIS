...
const releasedChecksReport = ({ reportsDb, getCheckTrailUseCase, reports }) => {
    return async function get(dateRange, SessionId){
        ...
        const result = []
        const fetch = await reports.getReleasedChecksReport(dateRange, SessionId);
        ...
        for await(check of fetch){
            ...
            data.transmittedBy = check.transmittedBy;
            data.receiveNumber = check.receiveNumber;
            data.receiveDate = check.receiveDate;
            data.releasedBy = check.releasedBy;
            data.releaseDate = check.releaseDate;
...
module.exports = releasedChecksReport;