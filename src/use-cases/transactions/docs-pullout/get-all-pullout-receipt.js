...
const { denyTransactionController } = require('../../../controllers/transactions/app');
const getAllPulloutReceiptsUseCase = ({transactions }) => {
    return async function get(dateRange,SessionId){
        ...
        if(dateRange.company == 'null'){
            dateRange.company=null
        }

        const fetched = await transactions.getAllPulloutReceipts(dateRange,SessionId);
        let data = [];
...
module.exports = getAllPulloutReceiptsUseCase;