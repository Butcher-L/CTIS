...
const receiveCheckUseCase = ({
  ...
  return async function post(info,SessionId) {
    ...
    //create receiveNumber
    receiveNumber = await createSeriesNumber(info.releaseLocationCode, transactions);
    info.receiveNumber = receiveNumber;

    const transaction = makeReceive(info);
    const checksArray = transaction.getChecks();
    var count = 0;

    const seriesNumber = await transactions.fix("CHK_RECEIVE");
...
POST /b1s/v1/U_CHK_RECEIVE

{   
  "Code": ${series},
  "Name": ${series},
  "U_RCV_NUM": "${info.receiveNumber}",
  "U_RCVD_BY": ${transaction.getReceiveBy()==undefined? null : transaction.getReceiveBy()},
  "U_RELEASER": ${transaction.getReleaser()},
  "U_CONTROL_COUNT": ${transaction.getControlCount()},
  "U_CONTROL_AMT": ${transaction.getControlAmount()},
  "U_DESC": "${transaction.getReceiveDescription()}",
  "U_RCV_DATE": "${moment().format()}",
  "U_DENIED": 0,
  "U_IS_POSTED": 1,
...
--b--
--a--`;
    // const postReceive = await transactions.createReceiveTransaction({
    //     Code: series,
    //     Name: series,
    //     U_RCV_NUM: info.receiveNumber,
    //     U_DESC: transaction.getReceiveDescription(),
    ...
    //     U_RCV_DATE: moment()
    // },SessionId);
    const postReceive = await checks.TransactionBatch(batchtns, SessionId)

    if (postReceive.data.includes('Bad Request')) {
      console.log(postReceive.data);
      throw new Error("Transaction failed in creating receive number")
    }

    const receiveId = series;
...
--a--`;
    ...
    return {
      msg: `Created receive transaction ${transaction.getReceiveNumber()} with ${count} check(s).`
    };
  };
...
module.exports = receiveCheckUseCase;
